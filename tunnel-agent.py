...
MQTT_SERVER_PORT_SSL = os.getenv('MQTT_SERVER_PORT_SSL')
MQTT_SERVER_USE_SSL = os.getenv('MQTT_SERVER_USE_SSL')
...
def get_mac_address(ifname):
    ...
    # 12-digit hex only
    return ''.join(binascii.hexlify(info[18:24]).decode('utf-8'))
...
def on_message(client, userdata, message):
    ...
    try:
        messasge_dict = json.loads(message_string)
    except json.JSONDecodeError as exc:
        logger.debug("MQTT JSON decode error: " + str(exc))
    else:
        logger.debug("MQTT JSON decoded: " + str(messasge_dict))
        if "tunnel" in messasge_dict and messasge_dict["tunnel"] == "open":
            if "port" in messasge_dict:
                remote_port_num = 0
                try:
                    remote_port_num = int(messasge_dict["port"])
                except:
        ...
        elif "tunnel" in messasge_dict and messasge_dict["tunnel"] == "close":
...
def on_subscribe(client, userdata, mid, granted_qos):
    logger.info("MQTT Subscribed to '" + MQTT_TOPIC_PREFIX_REMOTECONNECT + "/" + mac_address + "' QOS=" + str(granted_qos))
...
parser.add_argument('-p', '--port', help='Local port to forward to, e.g. "-p 80" for local httpd (default is 80)', required=False, type=int)
parser.add_argument('-v', '--version', help='Print version info', action='version', version=PROG_NAME + " version " + PROG_VERSION)
